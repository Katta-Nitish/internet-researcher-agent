...
@st.cache_resource
def image():
    return st.image("D:/git-hub projects/i1.png", width=200)
...
def final_output(state: State):
    ...
    prompt = (
        ...
        f"- THE CONFLICT DESCRIBED IS THE 2026 STRAIT OF HORMUZ CRISIS.\n"
        f"- DO NOT use the year '2024' or any internal memory from that time.\n"
        f"- If a source mentions a specific day (like March 13), assume it is from the year 2026.\n"
        f"- Use terms like 'Current,' 'Ongoing,' or 'Latest' for the timeline.\n\n"
        f"RESEARCH DATA:\n{state['embedding']}\n\n"
        f"TASK: Draft a report on: {state['query']}. Ensure all dates and sources reflect the 2026 context provided in the data."
    )
...
5.DO NOT mention the year 2024 or any other year from your internal training data.
'''
    )
if st.session_state.report:
